...
/** Body-portal dropdown with sane z-index scale */
function DropdownPortal({ open, anchorRef, onClose, width=260, children }) {
  ...
  useEffect(() => {
    ...
    const onClickAway = (e) => {
      ...
      const panel = document.getElementById('lb-dropdown-panel');
      if (panel && !panel.contains(e.target) && anchor && !anchor.contains(e.target)) onClose?.();
    };
  ...
  return createPortal(
    <div id="lb-dropdown-panel" style={style}>{children}</div>,
    document.body
  );
}
...
export default function NavBar() {
  ...
  return (
    <nav className="lb-nav">
      ...
      <style jsx>{`
        :root { --z-nav: 1000; --z-dropdown: 1100; }
        ...
        .lb-wrap{max-width:1200px;margin:0 auto;height:70px;display:flex;align-items:center;justify-content:space-between;padding:0 20px;}
        .lb-brand{font-weight:800;font-size:22px;color:#1e3a8a;}
        .lb-menu{display:none;gap:22px;align-items:center;}
        .lb-link{color:#1e40af;text-decoration:none;font-weight:600;padding:8px 10px;border-radius:6px;background:transparent;border:0;cursor:pointer;transition:.2s;}
